...
public class RegistroHuesped extends JFrame {
    ...
    public RegistroHuesped(Reservas reserva) {
        ...
        btnguardar.addMouseListener(new MouseAdapter() {
            @Override
            public void mouseClicked(MouseEvent e) {
                if (!txtNombre.getText().trim().equals("") && !txtApellido.getText().trim().equals("") && !txtTelefono.getText().trim().equals("")) {
                    if (saveHuesped()) {
                        Exito exito = new Exito("Datos guardados satisfactoriamente");
                        exito.setVisible(true);
                        dispose();
                    } else {
                        JOptionPane.showMessageDialog(null, "debes de ser mayor de 18 años para hacer una reservación");
                    }
                } else {
        ...
        txtNreserva.setText(String.valueOf(ctrl.getIdReserva() + 1));
    }

    private RegistroHuesped() {
    ...
    private boolean saveHuesped() {
        if (validateYear()) {
            ctrl.CreateReserva(this.reserva);
            Huespedes newHuesped = new Huespedes(
                    ctrl.getIdHuesped() + 1, txtNombre.getText().trim(), txtApellido.getText().trim(), txtNacionalidad.getSelectedItem().toString(), txtFechaN.getDate(), txtTelefono.getText().trim(), this.reserva);
            ctrl.CreateHuespedes(newHuesped);
            return true;
        } else {
            return false;
        }
    ...
    private boolean validateYear() {
        ...
        long milisegundosAño = (long) 365.25 * 24 * 60 * 60 * 1000;
        return diferenciaMilisegundos / milisegundosAño >= 18;
    }
}
